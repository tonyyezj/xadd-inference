...
% Keep this short and sweet... NIPS reviewers want to get to the
% technical contributions as fast as possible.  -Scott
Probabilistic inference in many real-world problems often requires
...
can be written as a product of some terms $t_{i,j,k}$ where the maximum degree of each variable in each $t_{i,j,k}$ is less or equal to 2. % (treating other variables as constants).%($\text{rel-degree}(t_{i,j,k}) \leq 4$)
...
SymGibbs, BaseGibbs and MH are run on \emph{collapsed-determinism} models while in the case of HMC and SMC, determinism is softened by observation noise. %(as required by Stan and Anglican PPLs).
